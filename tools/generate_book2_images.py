"""
Generate AI images for Book 2 (A2) units with Nano Banana 1
(google/gemini-2.5-flash-image) via OpenRouter.

Same shape as generate_book1_images.py / generate_book3_images.py — a unit
RANGE rather than the older one-script-per-unit generate_unitNN_images.py files
(which cover Book 2 units 9-30, and whose numbering collides with Book 1's
generate_unit1_images.py).
...
Usage:
    python tools/generate_book2_images.py <start_unit> <end_unit>
    e.g.  python tools/generate_book2_images.py 1 1

    IMAGE_WORKERS=1 python tools/generate_book2_images.py 1 5   (thin balance)
"""
...
# image model's content filter. Keyed by lowercase headword.
SAFE_PROMPTS = {}
...
def main():
    ...
    for un in range(start, end + 1):
        ...
        if not unit:
            continue
        out_dir = os.path.join("app", "static", "images", "book2", f"unit{un}")
        ...
        for w in db.query(Word).filter(Word.unit_id == unit.id).order_by(Word.position).all():
            ...
            tasks.append((
                ...
                f"/static/images/book2/unit{un}/{wl}.png",
                build_prompt(w.word, w.definition, w.example),
            ))
    db.close()

    # A finished unit may already be on disk as WebP (see compress_images_webp.py)
    def done_on_disk(path):
        return os.path.exists(path) or os.path.exists(os.path.splitext(path)[0] + ".webp")

    todo = [t for t in tasks if not done_on_disk(t[3])]
    ...
    # Update DB image_url for every word whose file now exists (idempotent).
    db = SessionLocal()
    updated = 0
    for un, wid, word, out_path, static_url, prompt in tasks:
        if os.path.exists(out_path):
            url = static_url
        elif os.path.exists(os.path.splitext(out_path)[0] + ".webp"):
            url = os.path.splitext(static_url)[0] + ".webp"
        else:
            continue
        w = db.get(Word, wid)
        if w and w.image_url != url:
            w.image_url = url
            updated += 1
    db.commit()
    db.close()
